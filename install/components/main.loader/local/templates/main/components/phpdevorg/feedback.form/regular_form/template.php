...
<form data-ms="ms_1" action="<?= POST_FORM_ACTION_URI ?>" class="equipment-form-body"
                        ...
                        if ($arItem['MULTIPLE'] === 'Y') { ?>
                            <div class="form-input">
                                <select multiple="multiple" name="<?= $arItem['CODE'] ?>[]"
                                    ...
                                    <option value="0"<?= empty($i) ? ' selected="selected"' : '' ?>>
                                        Выберите из списка
                                    </option>
                        ...
                        <? } else { ?>
                            <div class="form-input">
                                <select name="<?= $arItem['CODE'] ?>"
                                        class="form<?= !empty($arItem['REQUIRED_FIELD']) ? ' req' : '' ?>">
                                    <option value="0"<?= empty($arResult[$arItem['CODE']]) ? ' selected="selected"' : '' ?>>
                                        Выберите из списка
                                    </option>
                                    <? if (!empty($arItem['VALUE_LIST'])) { ?>
                                        <? foreach ($arItem['VALUE_LIST'] as $id => $value) { ?>
    ...
    <div class="form-button">
        <input type="submit" name="submit" class="form__btn btn fw"
